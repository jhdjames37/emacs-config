...
#+begin_src emacs-lisp
   ...
   (defvar my/doom-theme 'doom-fairy-floss
     "Which doom-theme to use. See https://github.com/doomemacs/themes?tab=readme-ov-file#theme-list")

   (defvar my/enabled-lang
     '(cpp cuda python python-jupyter js rust latex org markdown typst yaml dotenv dockerfile caddyfile bison)
     "Language extensions enabled.
       available: (cpp cuda python python-jupyter js rust latex org markdown typst yaml dotenv dockerfile caddyfile bison)")

   (defvar my/enabled-feat
     '()
     "Extended feature which requires external support, or breaks user experiences
      available: (ellama aidermacs pdf workspace)")
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'after-make-frame-functions
               (lambda (new-frame)
                 ...
                 ;; Vanilla Scroll-bar is not pretty TODO: scroll-bar replacement
...
#+begin_src elisp
  (use-package doom-themes
    :pin melpa ;; TODO: check upstream dependency.
...
#+begin_src emacs-lisp
  (use-package pdf-tools
    :when (member 'pdf my/enabled-feat)
...
As agent coding tools such as =Claude Code= and =Opencode= provides more advanced functions for AI programming,
I currently (2026.03) use them
...
Note aider can be installed according to the [[https://aider.chat/docs/install.html][instruction]] (Both locally and remotely.)

#+begin_src elisp
  (use-package aidermacs
    ;;:pin melpa-stable 
    :when (member 'aidermacs my/enabled-feat)
    :bind (("C-c a" . aidermacs-transient-menu))
    :custom
    ;;(aidermacs-default-model my/use-model) ;; defined in config.
    (aidermacs-extra-args `("--config"
                            "~/.config/aider/aider.conf.yml"))
    )

  ;; From https://github.com/MatthewZMD/aidermacs/issues/162
  (defun diff-buffers-to-second-with-smerge (buffer1 buffer2)
    "Run diff on contents of BUFFER1 and BUFFER2, put result in BUFFER2, and enable smerge-mode.
          The diff output will be in unified format which is compatible with smerge-mode."
    (interactive (list (read-buffer "Base buffer: ")
                       (read-buffer "Changes buffer: ")))
    (let ((content1 (with-current-buffer buffer1
                      (buffer-string)))
          (content2 (with-current-buffer buffer2
                      (buffer-string))))
      ;; Write contents to temp files for diff
      (let ((a (make-temp-file "diff-buffer1-"))
  ...
  ;;            :around (defun user/aidermacs-merge-override (oldfun &rest r)
  ;;                      (cl-letf (((symbol-function 'ediff-buffers) #'diff-buffers-to-second-with-smerge))
  ;;                        (apply oldfun r))))
#+end_src
*** Claude code
#+begin_src emacs-lisp
...
TODO: Auto persp for each ssh session, [[https://gist.github.com/Bad-ptr/1aca1ec54c3bdb2ee80996eb2b68ad2d#file-persp-projectile-auto-persp-el][Reference.]] Now Implements a =open-project= command for creating a workspace for a directory.

#+begin_src emacs-lisp
  (use-package persp-mode
    :when (member 'workspace my/enabled-feat)
    :custom
    (wg-morph-on nil)
    (persp-autokill-buffer-on-remove 'kill-weak)
    (persp-auto-resume-time -1)
    :pin melpa-stable  ;; current melpa release have compatible issue.
    :init
    (persp-mode 1)
...
In some scenarios, I use =eat= as a in-emacs terminal.

However, I need to figure out how to locate them efficiently when there is a lot of buffers.
  ...
  #+begin_src elisp
    ...
    (use-package eat
      ...
      ("<f11>" . eat)
      :config
      ;; Anti-flicker for TUI application.

      ;; https://codeberg.org/akib/emacs-eat/issues/110
      (setopt eat-very-visible-cursor-type '(t nil nil))


      ;; Here is the mitigation for vterm
      ;; Ref:   https://github.com/martinbaillie/vterm-anti-flicker-filter/
      ;;    and https://github.com/yuya373/claude-code-emacs.
      ;; However, this doesn't work for eat
      
      (defvar-local my/anti-flicker--output-buffer nil
        "Buffer for accumulating output during potential flickering sequences.")

      (defvar-local my/anti-flicker--output-timer nil
        "Timer for processing buffered output.")

      (defvar-local my/anti-flicker--original-filter nil
        "Original process filter function before anti-flicker was enabled.")

      (defun my/anti-flicker--flush-buffer (buffer)
        "Flush buffered output in BUFFER to the process."
        (when (buffer-live-p buffer)
          (with-current-buffer buffer
            (when my/anti-flicker--output-buffer
              (let ((data my/anti-flicker--output-buffer)
                    (proc (get-buffer-process buffer)))
                (setq my/anti-flicker--output-buffer nil
                      my/anti-flicker--output-timer nil)
                (when (and proc (process-live-p proc))
                  (funcall my/anti-flicker--original-filter proc data)))))))

      (defun my/anti-flicker--filter (process input)
        "Filter PROCESS INPUT to reduce flickering by buffering rapid redraws.
        ...
        (with-current-buffer (process-buffer process)
          ...
          (let ((has-clear-line (string-match-p "\033\\[K" input))
            ...
            (if (or (and (>= escape-count 3)
                    ...
                    my/anti-flicker--output-buffer)
                (message "!!!")
                (progn
                  ;; Buffer this output
                  (setq my/anti-flicker--output-buffer (concat my/anti-flicker--output-buffer input))
                  ;; Reset timer
                  (when my/anti-flicker--output-timer
                    (cancel-timer my/anti-flicker--output-timer))
                  (setq my/anti-flicker--output-timer
                        (run-at-time 0.016 nil #'my/anti-flicker--flush-buffer (current-buffer))))
              ;; Normal output, process immediately
              (funcall my/anti-flicker--original-filter process input)))))

      (defun my/anti-flicker-filter-enable ()
        "Enable anti-flicker filter for the current buffer.
    ...
    preserved and can be restored with `my/anti-flicker-filter-disable'.

    This function is automatically called for new buffers when
    the package is loaded."
        (interactive)
        (message "A")
        (when (derived-mode-p 'eat-mode) ;; NOTE: May remove it?
          (message "B")
          (when-let ((proc (get-buffer-process (current-buffer))))
            (unless my/anti-flicker--original-filter
              (setq my/anti-flicker--original-filter (process-filter proc)))
            (message "C")
            (set-process-filter proc #'my/anti-flicker--filter)
            (add-hook 'kill-buffer-hook
                      (lambda ()
                        (when my/anti-flicker--output-timer
                          (cancel-timer my/anti-flicker--output-timer))) nil t))))

      (defun my/anti-flicker-filter-disable ()
        "Disable anti-flicker filter for the current buffer.

    Restores the original process filter and flushes any
    buffered output immediately.  Cancels any pending timer."
        ...
        (when (derived-mode-p 'eat-mode)
          (when-let ((proc (get-buffer-process (current-buffer))))
            (when my/anti-flicker--output-timer
              (cancel-timer my/anti-flicker--output-timer)
              (setq my/anti-flicker--output-timer nil))
            (when my/anti-flicker--output-buffer
              (funcall my/anti-flicker--original-filter--filter proc my/anti-flicker--output-buffer)
              (setq my/anti-flicker--output-buffer nil))
            (set-process-filter proc (or my/anti-flicker--original-filter #'eat--filter))
            (setq my/anti-flicker--original-filter nil))))

      (defun my/anti-flicker-filter-toggle ()
        "Toggle anti-flicker filter for the current buffer.

    If the filter is currently enabled, disable it.  If disabled, enable it."
        (interactive)
        (if my/anti-flicker--original-filter
            (my/anti-flicker-filter-disable)
          (my/anti-flicker-filter-enable)))

      ;;(add-hook 'eat-mode-hook #'my/anti-flicker-filter-enable)
      )
  #+end_src
...
+ =atomic-chrome= (and its browser plugin) theoretically can use Emacs as editor in browsers (but losing loads of useful features).
  #+begin_src elisp
    (use-package atomic-chrome
      :when (member 'atomic-chrome my/enabled-feat)
      :init
...
+ =clipetty= allows integration for TTY-based emacs with global clipboard.
  It already checks whether in a TTY env, so no outside check is needed.
...
#+begin_src emacs-lisp
  ...
  (use-package jupyter
    :when (and (member 'python my/enabled-lang)
               (member 'python-jupyter my/enabled-lang)
               )
    )
#+end_src
